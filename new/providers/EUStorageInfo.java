    ...
    public void addInfo(IJadeHelper helper, BlockEntity blockEntity, Player player) {
        if (blockEntity instanceof BaseTileEntity baseTile) {
            if (baseTile instanceof CreativeSourceTileEntity) {
                ...
                bar(helper, (int) tp.getAvailableEnergy(), (int) tp.getMaxEnergy(), Component.translatable("ic2.probe.eu.storage.name", Formatter.formatInt((int) tp.getAvailableEnergy(), 4)).withStyle(ChatFormatting.WHITE), ColorUtils.CYAN);
            }
        }
    }
}
