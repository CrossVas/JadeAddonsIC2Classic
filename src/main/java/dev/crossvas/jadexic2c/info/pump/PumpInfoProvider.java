...
public enum PumpInfoProvider implements IHelper<BlockEntity> {
    ...
    public void appendTooltip(ITooltip iTooltip, BlockAccessor blockAccessor, IPluginConfig iPluginConfig) {
        if (!shouldAddInfo(blockAccessor, "PumpInfo", SpecialFilters.EU_READER)) {
            return;
        }
        ...
        if (blockAccessor.getBlockEntity() instanceof PumpTileEntity pump) {
            ...
            if (pumpProgress > 0) {
                BarHelper.bar(iTooltip, pumpProgress, pump.getPumpMaxProgress(),
                        Component.translatable("ic2.probe.progress.full.name", pumpProgress, pump.getPumpMaxProgress()).append(" t").withStyle(ChatFormatting.WHITE), -16733185);
            }
