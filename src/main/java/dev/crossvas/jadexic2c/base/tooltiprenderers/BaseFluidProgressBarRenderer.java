...
public class BaseFluidProgressBarRenderer implements IWailaTooltipRenderer {
    ...
    public void draw(@Nonnull String[] strings, @Nonnull IWailaCommonAccessor iWailaCommonAccessor) {
        ...
        if (tooltip != null) {
            ...
            long current = Integer.parseInt(strings[0]);
            long max = Integer.parseInt(strings[1]);
            int borderColor = Integer.parseInt(strings[2]);
            GuiHelper.THIS.renderTank((int) current, (int) max, x, y, width, height, borderColor, strings[3]);
