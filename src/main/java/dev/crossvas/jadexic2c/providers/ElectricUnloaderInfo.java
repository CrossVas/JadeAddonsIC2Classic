...
public class ElectricUnloaderInfo implements IInfoProvider {
    ...
    public void addInfo(IJadeHelper helper, BlockEntity blockEntity, Player player) {
        if (blockEntity instanceof BaseElectricUnloaderTileEntity unloader) {
            text(helper, "ic2.probe.eu.tier.name", EnergyNet.INSTANCE.getDisplayTier(unloader.getSourceTier()));
            text(helper, "ic2.probe.eu.output.max.name", unloader.getMaxEnergyOutput());
            text(helper, "ic2.probe.transformer.packets.name", 10);
