        ...
        if (blockEntity instanceof IWrenchableTile wrenchableTile) {
            ...
            if (actualRate > 0) {
                if (wrenchableTile.isHarvestWrenchRequired(player)) {
                    ...
                    text(helper, PluginHelper.getTextColorFromDropChance(100), Component.literal(100 + "% ").append(Component.translatable("ic2.probe.wrenchable.drop_chance.info").withStyle(ChatFormatting.GRAY)));
                    helper.addItemElement(wrenchStack, Component.translatable("ic2.probe.wrenchable.optional.info").withStyle(ChatFormatting.AQUA));
                }
            }
        }
    }
