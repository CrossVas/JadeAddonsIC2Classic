    ...
    public void addInfo(IJadeHelper helper, BlockEntity blockEntity, Player player) {
        if (blockEntity instanceof SimplePipePumpTileEntity simplePump) {
            if (simplePump instanceof ElectricPipePumpTileEntity electricPump) {
                text(helper, "ic2.probe.eu.tier.name", EnergyNet.INSTANCE.getDisplayTier(electricPump.getTier()));
                text(helper, "ic2.probe.eu.max_in.name", EnergyNet.INSTANCE.getPowerFromTier(electricPump.getTier()));
            }
            text(helper, "ic2.probe.pump.pressure", simplePump.getPressure());
            text(helper, "ic2.probe.pump.amount", Formatters.EU_FORMAT.format(simplePump.getDrainAmount() / 20));
        }
    }
}
